# All experimental data was flipped as if the mouse should always go to the left
...
head(mt[[1]])
mt[[2]][1:10,1:10,1] #Timestamps for different trajectories
...
#What did we get?
head(mt[[2]][1,,])
#We get th positions and precentage distange traveled at different timestamps
...
#what did we get?
head(measures) 
#Total distance = Euclidian distance
...
#simplest test possible to test difference in volatility
...
# Loss scenaria + Only the actual trials (Not the testruns)
...
mt_plot(spatial, use="trajectories", use2 = cluster_test, facet_col="cluster") #Gain scenaria
...
#### Could be used to have multiple plots in one window - doesn't seem to work though
